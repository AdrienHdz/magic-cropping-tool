"""
Code taken and modified from https://github.com/twitter-research/image-crop-analysis
...
class ImageSaliencyModel(object):
    ...
    def plot_crop_area(
        ...
        self.crop_coordinates.append({'x':x, 'y':y, 'w':w, 'h':h})
    ...
    def plot_img_crops(
        ...
        if aspectRatios is None:
            aspectRatios = [selected_width/img_w, img_h/selected_height]
        ...
        # Resizing input image based on coordinates
        img = img[int(self.crop_coordinates[0]['y']):int(self.crop_coordinates[0]['y']+int(self.crop_coordinates[0]['h'])),
         int(self.crop_coordinates[1]['x']):int(self.crop_coordinates[1]['x'])+int(self.crop_coordinates[1]['w'])]
